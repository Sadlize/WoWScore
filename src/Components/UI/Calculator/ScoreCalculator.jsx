...
const ScoreCalculator = () => {
    ...
    dungeonArray.forEach(item =>
        sumDungeonScoreValues += Object.values(scorePerDungeon[item]).reduce((a, b) => (a * 1.5) + (b * 0.5))
    )
    ...
    const calcPointsForKeyLevel = (keyLevel) => {
        ...
        return 30 + (keyLevel * 5) + (affixes * 5) + seasonal
    }
                        ...
                        <input
                            ...
                            onChange={event =>
                                setScorePerDungeon(prevState => (
                                    {...prevState, [key]: {...prevState[key], 'Tyrannical': calcPointsForKeyLevel(+event.target.value)}}
                                ))}
                        />
                        <input
                            placeholder='0'
                            ...
                            onChange={event =>
                                setScorePerDungeon(prevState => (
                                    {...prevState, [key]: {...prevState[key], 'Fortified': calcPointsForKeyLevel(+event.target.value)}}
                                ))}
                        />
                    </div>
                ))}
